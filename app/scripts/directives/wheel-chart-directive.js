angular.module('wheelchartApp')
    .directive('wheelChart', [function(){
		...
		function buildObj(node, depth){
		  ...
		  if(node.father){
		    obj.children.push(buildObj(node.father, depth + 1));
		  }
		  if(node.mother){
		    obj.children.push(buildObj(node.mother, depth + 1));
		  }
		  return obj;
		}
        ...
        return {
            ...
            link: function(scope, element, attrs, ngModel){
            	console.log(attrs);
                //because this is using the pedigree data object which has .father and .mother properties
                //but d3 requires a .children property we ne to tranform the shape of the object to fit
				var root = buildObj(scope.pedigreeData, 1);
				console.log(root);
				...
				var radius = 350, numRounds = maxDepth, segmentWidth = radius / (numRounds + 1);
				...
				svg.append('g').attr('transform', translateSVG(xCenter,yCenter));
				...
				//d3.json('data/tree.json', function(err, root) {
				  ...
				  // Round labels
				  var rounds = ['Round 1', 'Round 2', 'Round 3', 'Round 4', 'Quarter finals', 'Grandparents', 'Parents'];
